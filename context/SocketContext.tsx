...
export const SocketProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const addToast = useCallback(
    (toast: Omit<ToastAlert, "id" | "timestamp">) => {
  ...
  const initSocket = useCallback(() => {
    ...
    socketInstance.on("error", (err: SocketErrorPayload) => {
      ...
      // Provide user-friendly message for match not found errors
      if (
        ...
        addToast({
          ...
          description:
            "This match is no longer available. Routing to home page...",
          type: "error",
        });
      } else {
        addToast({
          title: "Notice",
          description:
            err.message || "An error occurred on the live connection.",
          type: "error",
        });
      }
    });
  ...
  const sendChatMessage = useCallback(
    (matchId: string, userId: string, username: string, message: string) => {
      if (socket && socket.connected) {
